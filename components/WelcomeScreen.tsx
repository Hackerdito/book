...
const WelcomeScreen: React.FC<WelcomeScreenProps> = ({ onEnter, isExiting }) => {
  ...
  const handleIngresarClick = () => {
    // Modificación: Ahora entramos directamente para ver el nuevo diseño
    onEnter();
    // Si quieres volver a mostrar el mensaje de "Próximamente", descomenta la línea de abajo y comenta onEnter()
    // setShowModal(true);
  };
  ...
  return (
    ...
    >
      ...
      <div className={`absolute bottom-4 text-white/60 text-xs font-light tracking-widest uppercase transition-opacity ${showModal ? 'opacity-0' : 'opacity-100'}`}>
        © 2026 Gerito DG
      </div>
